...
public class Person {
	...
	public Person(short yearOfBirth, String name, String surname) {
		/*
		 * Solución con if if(yearOfBirth < 1900){ yearOfBirth = 1900; } else
		 * if(yearOfBirth > 2023) { yearOfBirth = 2023; } this.yearOfBirth =
		 * yearOfBirth;
		 */
	...
	public void show() {
		/*
		 * System.out.printf("Person %s %s de %d años.%n", name, surname, 2023 -
		 * yearOfBirth);
		 */
		System.out.printf("Person %s(%b) %s(%b) de %d años.%n", name, brokenName, surname, brokenSurname,
				2023 - yearOfBirth);
	}
	...
	public boolean compareWith(Person p) {
		...
		return yearOfBirth == p.yearOfBirth && name == p.name && surname == p.surname && brokenName == p.brokenName
